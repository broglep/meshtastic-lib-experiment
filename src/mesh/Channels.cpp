 ...
 */
int16_t Channels::generateHash(meshtastic_Config_LoRaConfig *loraConfig, ChannelIndex channelNum)
{
    auto k = getKey(channelNum);
    if (k.length < 0)
        return -1; // invalid
    else {
        const char *name = getName(loraConfig, channelNum);
 ...
 */
meshtastic_Channel &Channels::fixupChannel(meshtastic_Config_LoRaConfig *loraConfig, ChannelIndex chIndex)
{
    ...
    hashes[chIndex] = generateHash(loraConfig, chIndex);

    return ch;
}

void Channels::initDefaultLoraConfig(meshtastic_Config_LoRaConfig *loraConfig)
{
...
CryptoKey Channels::getKey(ChannelIndex chIndex)
{
    ...
    if (!ch.has_settings || ch.role == meshtastic_Channel_Role_DISABLED) {
        LOG_WARN("Channel %s is disabled", ch.settings.name);
        k.length = -1; // invalid
    } else {
...
int16_t Channels::setCrypto(ChannelIndex chIndex)
{
    ...
    LOG_WARN("Crypt Key Length %d", k.length);
    if (k.length < 0)
        return -1;
    else {
...
void Channels::initDefaults(meshtastic_Config_LoRaConfig *loraConfig)
{
    channelFile.channels_count = MAX_NUM_CHANNELS;
    for (int i = 0; i < channelFile.channels_count; i++)
        fixupChannel(loraConfig, i);
    initDefaultLoraConfig(loraConfig);
...
void Channels::onConfigChanged(meshtastic_Config_LoRaConfig *loraConfig)
{
    // Make sure the phone hasn't mucked anything up
    for (int i = 0; i < channelFile.channels_count; i++) {
        const meshtastic_Channel &ch = fixupChannel(loraConfig, i);
...
meshtastic_Channel &Channels::getByName(meshtastic_Config_LoRaConfig *loraConfig, const char *chName)
{
    for (ChannelIndex i = 0; i < getNumChannels(); i++) {
        if (strcasecmp(getGlobalId(loraConfig, i), chName) == 0) {
...
const char *Channels::getName(meshtastic_Config_LoRaConfig *loraConfig, size_t chIndex)
{
    ...
    if (!*channelName) { // emptystring
        ...
        if (loraConfig->use_preset) {
            channelName = DisplayFormatters::getModemPresetDisplayName(loraConfig->modem_preset, false);
        } else {
            channelName = "Custom";
        }
    }
...
bool Channels::isDefaultChannel(meshtastic_Config_LoRaConfig *loraConfig, ChannelIndex chIndex)
{
    const auto &ch = getByIndex(chIndex);
    if (ch.settings.psk.size == 1 && ch.settings.psk.bytes[0] == 1) {
        const char *name = getName(loraConfig, chIndex);
        const char *presetName = DisplayFormatters::getModemPresetDisplayName(loraConfig->modem_preset, false);
...
bool Channels::hasDefaultChannel(meshtastic_Config_LoRaConfig *loraConfig)
{
    // If we don't use a preset or the default frequency slot, or we override the frequency, we don't have a default channel
    if (!loraConfig->use_preset || !RadioInterface::uses_default_frequency_slot || loraConfig->override_frequency)
        return false;
    // Check if any of the channels are using the default name and PSK
    for (size_t i = 0; i < getNumChannels(); i++) {
        if (isDefaultChannel(loraConfig, i))
            return true;
    }
    return false;
}
...
ChannelIndex Channels::getNumChannels() {
    return channelFile.channels_count;
}
