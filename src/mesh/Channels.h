...
class Channels
{
  ...
  public:
    ...
    meshtastic_Channel &getByName(meshtastic_Config_LoRaConfig *loraConfig, const char *chName);
     ...
     */
    const char *getName(meshtastic_Config_LoRaConfig *loraConfig, size_t chIndex);

    /**
     * Return a globally unique channel ID usable with MQTT.
     */
    const char *getGlobalId(meshtastic_Config_LoRaConfig *loraConfig, size_t chIndex) { return getName(loraConfig, chIndex); } // FIXME, not correct
    ...
    void initDefaults(meshtastic_Config_LoRaConfig *loraConfig);

    /// called when the user has just changed our radio config and we might need to change channel keys
    void onConfigChanged(meshtastic_Config_LoRaConfig *loraConfig);
    ...
    bool isDefaultChannel(meshtastic_Config_LoRaConfig *loraConfig, ChannelIndex chIndex);

    // Returns true if we can be reached via a channel with the default settings given a region and modem preset
    bool hasDefaultChannel(meshtastic_Config_LoRaConfig *loraConfig);
  ...
  private:
     ...
     */
    int16_t generateHash(meshtastic_Config_LoRaConfig *loraConfig, ChannelIndex channelNum);
     ...
     */
    meshtastic_Channel &fixupChannel(meshtastic_Config_LoRaConfig *loraConfig, ChannelIndex chIndex);

    /**
     * Writes the default lora config
     */
    void initDefaultLoraConfig(meshtastic_Config_LoRaConfig *loraConfig);
