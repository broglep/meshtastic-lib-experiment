...
uint32_t Default::getConfiguredOrDefaultMs(uint32_t configuredInterval, meshtastic_Config_DeviceConfig *deviceConfig)
{
    if (configuredInterval > 0)
        return configuredInterval * 1000;
    return default_broadcast_interval_secs(deviceConfig) * 1000;
}
 ...
 */
uint32_t Default::getConfiguredOrDefaultMsScaled(meshtastic_Config_DeviceConfig *deviceConfig,
                                                 ...
                                                 uint32_t defaultValue, uint32_t numOnlineNodes)
{
    ...
    if (IS_ONE_OF(deviceConfig->role, meshtastic_Config_DeviceConfig_Role_SENSOR, meshtastic_Config_DeviceConfig_Role_TRACKER))
        return getConfiguredOrDefaultMs(configured, defaultValue);

    return getConfiguredOrDefaultMs(configured, defaultValue) * congestionScalingCoefficient(loraConfig, numOnlineNodes);
}
...
uint8_t Default::getConfiguredOrDefaultHopLimit(meshtastic_Config_LoRaConfig *loraConfig, uint8_t configured)
{
#if USERPREFS_EVENT_MODE
    return (configured > HOP_RELIABLE) ? HOP_RELIABLE : config.lora.hop_limit;
#else
    return (configured >= HOP_MAX) ? HOP_MAX : loraConfig->hop_limit;
#endif
}