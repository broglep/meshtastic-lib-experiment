...
bool MeshPacketQueue::_isFromUs(const meshtastic_MeshPacket *p)
{
    return p->from == 0 || p->from == *nodeId;
}

/// @return "true" if "p1" is ordered before "p2"
bool MeshPacketQueue::CompareMeshPacketFunc(const meshtastic_MeshPacket *p1, const meshtastic_MeshPacket *p2)
{
    assert(p1 && p2);
    ...
    return (p1p != p2p) ? (p1p > p2p) : (!_isFromUs(p1) && _isFromUs(p2));
}

MeshPacketQueue::MeshPacketQueue(size_t _maxLen, NodeNum *_nodeId) : maxLen(_maxLen), nodeId(_nodeId) {
...
bool MeshPacketQueue::enqueue(meshtastic_MeshPacket *p)
{
    ...
    auto comparator = std::bind(&MeshPacketQueue::CompareMeshPacketFunc, this, std::placeholders::_1, std::placeholders::_2);
    auto it = std::upper_bound(queue.begin(), queue.end(), p, comparator);
...
meshtastic_MeshPacket *MeshPacketQueue::remove(NodeNum from, PacketId id, bool tx_normal, bool tx_late)
{
    for (auto it = queue.begin(); it != queue.end(); it++) {
        auto p = (*it);
        if (_getFrom(p) == from && p->id == id && ((tx_normal && !p->tx_after) || (tx_late && p->tx_after))) {
            queue.erase(it);
            return p;
        }
    }
...
NodeNum MeshPacketQueue::_getFrom(const meshtastic_MeshPacket *p) {
    return (p->from == 0) ? *nodeId : p->from;
}
