...
void initRegion(meshtastic_Config_LoRaConfig_RegionCode *region)
{
    ...
    for (; r->code != meshtastic_Config_LoRaConfig_RegionCode_UNSET && r->code != REGULATORY_LORA_REGIONCODE; r++)
        ;
    LOG_INFO("Wanted region %d, regulatory override to %s", *region, r->name);
#else
    for (; r->code != meshtastic_Config_LoRaConfig_RegionCode_UNSET && r->code != region; r++)
        ;
    LOG_INFO("Wanted region %d, using %s", region, r->name);
#endif
    myRegion = r;
}
...
uint32_t RadioInterface::getTxDelayMsecWeighted(meshtastic_Config_DeviceConfig *deviceConfig, float snr)
{
    ...
    if (deviceConfig->role == meshtastic_Config_DeviceConfig_Role_ROUTER ||
        deviceConfig->role == meshtastic_Config_DeviceConfig_Role_REPEATER) {
...
void printPacket(const char *prefix, const meshtastic_MeshPacket *p)
{
    ...
    std::string out = mt_sprintf("%s (id=0x%08x fr=0x%08x to=0x%08x, WantAck=%d, HopLim=%d Ch=0x%x", prefix, p->id,
                                            p->from, p->to, p->want_ack, p->hop_limit, p->channel);
    if (p->which_payload_variant == meshtastic_MeshPacket_decoded_tag) {
        auto &s = p->decoded;

        out += mt_sprintf(" Portnum=%d", s.portnum);

        if (s.want_response)
            out += mt_sprintf(" WANTRESP");

        if (p->pki_encrypted)
            out += mt_sprintf(" PKI");

        if (s.source != 0)
            out += mt_sprintf(" source=%08x", s.source);

        if (s.dest != 0)
            out += mt_sprintf(" dest=%08x", s.dest);

        if (s.request_id)
            out += mt_sprintf(" requestId=%0x", s.request_id);

        /* now inside Data and therefore kinda opaque
        if (s.which_ackVariant == SubPacket_success_id_tag)
            out += mt_sprintf(" successId=%08x", s.ackVariant.success_id);
        else if (s.which_ackVariant == SubPacket_fail_id_tag)
            out += mt_sprintf(" failId=%08x", s.ackVariant.fail_id); */
    } else {
        out += " encrypted";
        out += mt_sprintf(" len=%d", p->encrypted.size + sizeof(PacketHeader));
    }

    if (p->rx_time != 0)
        out += mt_sprintf(" rxtime=%u", p->rx_time);
    if (p->rx_snr != 0.0)
        out += mt_sprintf(" rxSNR=%g", p->rx_snr);
    if (p->rx_rssi != 0)
        out += mt_sprintf(" rxRSSI=%i", p->rx_rssi);
    if (p->via_mqtt != 0)
        out += mt_sprintf(" via MQTT");
    if (p->hop_start != 0)
        out += mt_sprintf(" hopStart=%d", p->hop_start);
    if (p->next_hop != 0)
        out += mt_sprintf(" nextHop=0x%x", p->next_hop);
    if (p->relay_node != 0)
        out += mt_sprintf(" relay=0x%x", p->relay_node);
    if (p->priority != 0)
        out += mt_sprintf(" priority=%d", p->priority);
...
bool RadioInterface::reconfigure(meshtastic_Config_LoRaConfig *loraConfig)
{
    applyModemConfig(loraConfig);
    return true;
}

bool RadioInterface::init(meshtastic_Config_LoRaConfig *loraConfig)
{
 ...
 */
void RadioInterface::applyModemConfig(meshtastic_Config_LoRaConfig *loraConfig)
{
    ...
    if ((power == 0) || ((power + REGULATORY_GAIN_LORA > myRegion->powerLimit)))
    ...
    const char *channelName = channels.getName(loraConfig, channels.getPrimaryIndex());
    ...
    RadioInterface::uses_default_frequency_slot =
        channel_num == hash(DisplayFormatters::getModemPresetDisplayName(loraConfig->modem_preset, false)) % numChannels;
