...
class RadioInterface
{
  ...
  public:
    ...
    virtual bool init(meshtastic_Config_LoRaConfig *loraConfig);

    /// Apply any radio provisioning changes
    /// Make sure the Driver is properly configured before calling init().
    /// \return true if initialisation succeeded.
    virtual bool reconfigure(meshtastic_Config_LoRaConfig *loraConfig);
    ...
    uint32_t getTxDelayMsecWeighted(meshtastic_Config_DeviceConfig *deviceConfig, float snr);
  ...
  private:
     ...
     */
    void applyModemConfig(meshtastic_Config_LoRaConfig *loraConfig);
    ...
    int reloadConfig(meshtastic_Config_LoRaConfig *loraConfig)
    {
        reconfigure(loraConfig);
        return 0;
    }
};
