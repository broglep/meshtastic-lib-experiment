...
RadioLibInterface::RadioLibInterface(LockingArduinoHal *hal, RADIOLIB_PIN_TYPE cs, RADIOLIB_PIN_TYPE irq, RADIOLIB_PIN_TYPE rst,
                                     RADIOLIB_PIN_TYPE busy, PhysicalLayer *_iface, NodeNum *_nodeId, meshtastic_Config_DeviceConfig *_deviceConfig)
    : NotifiedWorkerThread("RadioIf"), module(hal, cs, irq, rst, busy), iface(_iface), nodeId(_nodeId), txQueue(MeshPacketQueue(MAX_TX_QUEUE, _nodeId)), deviceConfig(_deviceConfig)
{
    instance = this;
...
void RadioLibInterface::setTransmitDelay()
{
    ...
    if (p->tx_after) {
        unsigned long add_delay = p->rx_rssi ? getTxDelayMsecWeighted(deviceConfig, p->rx_snr) : getTxDelayMsec();
...
void RadioLibInterface::startTransmitTimerSNR(float snr)
{
    // If we have work to do and the timer wasn't already scheduled, schedule it now
    if (!txQueue.empty()) {
        uint32_t delay = getTxDelayMsecWeighted(deviceConfig, snr);
        notifyLater(delay, TRANSMIT_DELAY_COMPLETED, false); // This will implicitly enable
    }
}
...
void RadioLibInterface::handleReceiveInterrupt()
{
    ...
    if (state != RADIOLIB_ERR_NONE) {
        ...
        if (payloadLen < 0) {
            ...
            rxGood++;
            LOG_DEBUG("Received packet, length=%d", payloadLen);
...
bool RadioLibInterface::_isFromUs(meshtastic_MeshPacket *p) {
