...
SX126xInterface<T>::SX126xInterface(LockingArduinoHal *hal, RADIOLIB_PIN_TYPE cs, RADIOLIB_PIN_TYPE irq, RADIOLIB_PIN_TYPE rst,
                                    RADIOLIB_PIN_TYPE busy, NodeNum *_nodeId, meshtastic_Config_DeviceConfig *deviceConfig)
    : RadioLibInterface(hal, cs, irq, rst, busy, &lora, _nodeId, deviceConfig), lora(&module)
{
...
template <typename T> bool SX126xInterface<T>::init(meshtastic_Config_LoRaConfig *loraConfig)
{
...
// means this workaround is not necessary.
    ...
    RadioLibInterface::init(loraConfig);
...
#endif
    if (_loraConfig->sx126x_rx_boosted_gain) {
...
template <typename T> bool SX126xInterface<T>::reconfigure(meshtastic_Config_LoRaConfig *loraConfig)
{
    RadioLibInterface::reconfigure(loraConfig);
